...
def write_report(stats, plan_rows, dry_run: bool):
    ...
    L.append("**Source table:** `public.thoughts` → `public.knowledge`")
    L.append("**Source tag:** `cutover:thoughts:<thoughts_id>` (one tag per row, for rollback)")
    L.append("")
...
def main() -> None:
    ...
    with psycopg.connect(_conninfo(_db_url()), row_factory=dict_row) as conn:
        ...
        print("Rollback: DELETE FROM public.knowledge WHERE source LIKE 'cutover:%';")
